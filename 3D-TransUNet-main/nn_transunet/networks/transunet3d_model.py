...
from nn_transunet.networks.vit_modeling import Transformer
from nn_transunet.networks.vit_modeling import CONFIGS as CONFIGS_ViT
...
if __name__ == '__main__':
    # 配置IBSR18数据尺寸 (256, 128, 256)
    input_size_3d = (128, 128, 128)  # D, H, W
    input_channels = 4
    
    # 创建3D模型（使用修复后的decoder_channels）
    model_3d = Generic_TransUNet_max_ppbp(
        ...
        pool_op_kernel_sizes=default_dict["net_num_pool_op_kernel_sizes"],
        conv_kernel_sizes=default_dict["net_conv_kernel_sizes"],
        patch_size=(64, 64, 64),
        # 显式设置decoder_channels以匹配实际特征图
        decoder_channels=[320, 256, 128, 64, 32, 32]  # 关键修复
    ).cuda()
    
    print(f"{'*' * 30} 3D模型分析 {'*' * 30}")
    
    # 计算参数量
    total_params = sum(p.numel() for p in model_3d.parameters() if p.requires_grad)
    print(f"3D模型总参数量: {total_params / 1e6:.2f} M")
    
    # 计算FLOPs
    try:
        input_tensor = torch.randn((1, input_channels, *input_size_3d)).cuda()
        flops, _ = profile(model_3d, inputs=(input_tensor,), verbose=False)
        print(f"3D模型FLOPs: {flops / 1e9:.2f} G")
    except ImportError:
        print("thop库未安装，跳过FLOPs计算")
    except RuntimeError as e:
        print(f"FLOPs计算时出错: {e}")